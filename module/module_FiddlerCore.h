...
//@日期 2022-11-12
//@作者 欧阳逸洛 
//@模块备注 FiddlerCore部分包装
//由于炫语言缺陷，无法自动复制文件，需要手动从模块目录复制 "Tools\Brotli.exe" 到程序目录，用于支持Brotli解码。
//包装源：https://www.telerik.com/fiddlercore https://gitee.com/ouyang-yiluo/fiddler-wrap

//@依赖 module_base.h

#ifdef _WIN64 //x64
//@复制文件 @当前模块路径 "x64/FiddlerCWrap_Extend.dll"
#else // x86 
//@复制文件 @当前模块路径 "x86/FiddlerCWrap_Extend.dll"
#endif

//@隐藏{
#pragma once
	
#ifndef FiddlerCore_H
#define FiddlerCore_H
//#include <windows.h>
#include <tchar.h>
#include <functional>
//#include <string>
...
namespace {
	...
	inline CXBytes WrapBytesPtrToCXBytes(WrapBytesPtr wbp) {
		CXBytes bytes;
		if (wbp) {
			bytes.set(const_cast<uint8_t*>(wbp->p), static_cast<uint32_t>(wbp->len));
		}
		return bytes;
	}
	inline WrapBytes CXBytesToWrapBytes(CXBytes& bytes) {
		return { reinterpret_cast<const uint8_t*>(bytes.get()), static_cast<ptrdiff_t>(bytes.size())};
	}
//@隐藏}
	...
	public:
		...
		//@别名 正则替换响应(用于搜索正文的正则表达式，用于替换的文本或表达式)
