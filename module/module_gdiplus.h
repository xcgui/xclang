#ifndef _XC_GDIPLUSYTX_H
//@隐藏{
#define _XC_GDIPLUSYTX_H
...
class GPImage : public GPbase {
	...
	bool FromFile(CXText text,bool IMC){
		if(IMC) 
	...
	//@别名 取帧数()
	UINT GetFrameCount(__in const GUID* dimensionID)
	{
		UINT count = 0;
		SetStatus(DllExports::GdipImageGetFrameCount(hImage,
															dimensionID,
															&count));
		return count;
	}
...
class GPGraphics : public GPbase {
	...
	//@别名 画圆角矩形(GP画笔,矩形,圆角度)
	bool DrawRoundedRect(GPPen& pen, Rect& rect, int cornerRadius = 10);
	//@别名 画曲线(GP画笔,坐标数组,个数)
	...
	//@别名 填充圆角矩形(GP画刷,矩形,圆角度)
	bool FillRoundedRect(GPBrush& brush, Rect& rect, int cornerRadius =10);
...
class GPBitmap : public GPImage {
	...
	//@别名 应用效果()
	Status ApplyEffect(GPEffect *effect,RECT *ROI)
	{
		if(effect->auxData)
		{
